...
double * bottomRow(int m, int n, double b[m], double a[m][n+m], double c[n+m]){
	int i, j;	
	double * delta = (double *)malloc((n+m) * sizeof(double));
	for(i = 0; i < n+m; ++i){
		delta[i] = -c[i];
		for(j = 0; j < n+m; ++j){
			if(isBasic[j])
...
int minIndex(double * delta, int size){
	int i, index = 0;
	for(i = 1; i < n; ++i){
...
void simplex(int m, int n, double a[m][n+m], double b[m]){

	int i, j;			
	double ratio[m];
	int min_ratio_index; 
	double * delta = bottomRow(m, n, b, a, c);
	int minindex = minIndex(delta, n+m);

	if(isBasic[minindex])	return;

	for(i = 0; i < m; ++i){
		ratio[i] = b[i] / a[minindex][i];
	}
	min_ratio_index = minIndex(ratio, m);
	isBasic[min_ratio_index] = 
}
...
int main(){
	...
	int isBasic[n+m];
	for(i = 0; i < n; ++i)	isBasic[i] = 0;
	for(i = n; i < n+m; ++i)	isBasic[i] = 1;
